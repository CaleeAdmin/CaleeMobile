    ...
    return Scaffold(
      ...
      body: Obx(() {
        ...
        if (ctrl.error.value.isNotEmpty) {
          return Center(child: Padding(padding: const EdgeInsets.all(16), child: Text('Failed to load subscriptions: ${ctrl.error.value}')));
        }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('${category.calendars.length} calendar${category.calendars.length > 1 ? "s" : ""} available', style: _metaStyle),
...
class _CalendarCard extends StatelessWidget {
  ...
  const _CalendarCard({Key? key, required this.calendar}) : super(key: key);
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 8),
                        ElevatedButton(
                        onPressed: calendar.icsUrl == null || calendar.icsUrl?.isEmpty == true ? null : () async {
                          final ok = await Get.find<CalendarPageController>().subscribePublicIcs(calendar.icsUrl!);
                          if (ok) {
                            Get.snackbar('Subscribed', 'Subscribed to "${calendar.title}"', snackPosition: SnackPosition.BOTTOM);
                            Navigator.of(context).pop(true);
                          } else {
                            Get.snackbar('Subscribe failed', 'Failed to subscribe to "${calendar.title}"', snackPosition: SnackPosition.BOTTOM, backgroundColor: Colors.red.shade50);
                  ...
                  if (calendar.description.isNotEmpty)
                    Text(calendar.description, style: _cardBodyStyle, maxLines: 3, overflow: TextOverflow.ellipsis),
                  const SizedBox(height: 12),
                  Row(
                    children: [
                      Column(
