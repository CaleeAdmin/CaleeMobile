...
void main() {
  ...
  group('monthly RRULE', () {
    ...
    test('monthly occurrences preserve day-of-month from DTSTART', () {
      ...
      final start = baseDate.isBefore(now.subtract(const Duration(days: 30)))
          ? DateTime(now.year, now.month, 5, 10, 0, 0)
              .add(const Duration(days: 1)) // fallback to tomorrow-ish
          : baseDate;
  ...
  group('window enforcement', () {
    test('event 400 days in the future is excluded', () {
      final farFuture = DateTime(now.year, now.month, now.day, 10, 0, 0)
    ...
    test('RRULE occurrences past windowEnd are not included', () {
      ...
      for (final e in events) {
        expect(e.start.isBefore(windowEnd.add(const Duration(days: 1))), isTrue,
  ...
  group('robustness', () {
    ...
    test('unsupported FREQ does not crash and returns empty', () {
      final ics = icsWrap([
        'BEGIN:VEVENT',
      ...
      expect(() => service.parseBody(ics, fakeSub()), returnsNormally);
      // YEARLY is unsupported — no occurrences emitted.
      expect(service.parseBody(ics, fakeSub()), isEmpty);
    });
