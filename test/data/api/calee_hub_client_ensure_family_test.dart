...
void main() {
  group('CaleeHubClient.ensureDefaultFamily', () {
    ...
    test('uses /households/default first and returns household from data.household', () async {
      final paths = <String>[];
      ...
      expect(paths, ['/client/v1/households/default'],
          reason: 'Should try idempotent endpoint first');
      expect(result.id, 'h1');
      expect(result.name, 'My Family');
    });

    test('falls back to /households when /households/default returns 404', () async {
      int callCount = 0;
      final paths = <String>[];
      ...
      expect(paths,
          ['/client/v1/households/default', '/client/v1/households'],
          reason: 'Should fall back to create endpoint on 404');
      expect(result.id, 'h2');
    });

    test('falls back to /households when /households/default returns 405', () async {
      int callCount = 0;
      ...
      server.listen((req) async {
        ...
        if (callCount == 1) {
          ...
          req.response
              .write(jsonEncode({'error': {'message': 'Not allowed'}}));
        } else {
          req.response.statusCode = HttpStatus.ok;
    ...
    test('accepts flat data response shape (no household/context key)', () async {
      server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      server.listen((req) async {
        req.response.statusCode = HttpStatus.ok;
        req.response.headers.contentType = ContentType.json;
    ...
    test('rethrows non-404/405 errors from /households/default without falling back', () async {
      server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      server.listen((req) async {
        req.response.statusCode = HttpStatus.internalServerError;
        req.response.headers.contentType = ContentType.json;
        req.response
            .write(jsonEncode({'error': {'message': 'Internal error'}}));
        await req.response.close();
      });
    ...
    test('throws CaleeHubException when response data is not a valid context', () async {
      server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      server.listen((req) async {
        req.response.statusCode = HttpStatus.ok;
        req.response.headers.contentType = ContentType.json;
